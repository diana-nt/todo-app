<template>
  <div class="home">
      <addTarea @add-tarea="addTarea" />

      <custom-select
      :options="['Tareas', 'Pendientes', 'Finalizadas']"
      default='Tareas'
      class="select"
      @input="changeTab"/>

    <tareas
        :tareas="tareasMostradas()"
        @completar-tarea="completarTarea"
        @delete-tarea="deleteTarea"></tareas>

    <button @click="ordenarTareas">Ordenar</button>
  </div>
</template>

<script>
import Tareas from "@/components/Tareas";
import AddTarea from "@/components/AddTarea";
import CustomSelect from "@/components/CustomSelect";
import repo from "@/services/tasks.service";

export default {
  name: 'Home',
  components: {
    Tareas,
    AddTarea,
      CustomSelect
  },
  data() {
    return {
      actual: 'tareas',
      tareas: [
        /*{
          id: 1,
          title: 'Aprender Vue',
          completed: false,
          created_on: new Date('2017-06-01')
        },
        {
          id: 2,
          title: 'Lavar coche',
          completed: false,
          created_on: new Date('2018-04-23')
        },
        {
          id: 3,
          title: 'Pasar la aspiradora',
          completed: false,
          created_on: new Date('2019-11-30')
        },
        {
          id: 4,
          title: 'Hacer trabajo de clase',
          completed: false,
          created_on: new Date('2019-02-10')
        },
        {
          id: 5,
          title: 'Comprar impresora',
          completed: false,
          created_on: new Date('2004-08-13')
        }*/
      ],

    }
  },
  created() {
    this.getTasksFromRepository();
  },
  methods: {
    changeTab(tab){
      this.actual = tab.toLowerCase();
    },
      tareasMostradas(){
        if (this.actual === 'finalizadas'){
            return this.tareasFinalizadas
        }else if (this.actual === 'pendientes'){
            return this.tareasPendientes
        }
          return this.tareas;
      },

    addTarea(nuevaTarea) {
      this.tareas = [...this.tareas, nuevaTarea];
    },
    deleteTarea(tareaId){
      this.tareas = this.tareas.filter(tarea => tarea.id !== tareaId);
    },
    ordenarTareas(){
      return this.tareas.sort((a, b) => new Date(a.created_on) - new Date(b.created_on))
    },
    completarTarea(tareaId){
        let t = this.tareas.find(tarea => tarea.id === tareaId);
        t.completed = !t.completed;
    },
    async getTasksFromRepository() {
        let tasks = await repo.getTasks();
        this.$set(this, "tareas", tasks);
    }

  },
  computed:{
    tareasFinalizadas(){
      return this.tareas.filter(tarea => tarea.completed === true);
    },
    tareasPendientes(){
      return this.tareas.filter(tarea => tarea.completed === false);
    }
  },
}
</script>

<style>

.checkmark {
    position: absolute;
    top: 0;
    left: 0;
    height: 18px;
    width: 18px;
    /*background-color: azure;*/
    border: 1px solid black;
}

</style>